...
def read_data(filenm, colnm):
    inputfile = open(filenm, 'rb')  # 可打开含有中文的地址
    df = pd.read_csv(inputfile, usecols=['time', 'up', 'down', 'total'])
    t = df['time'].values
    y = df[colnm].values
...
def windowed_dataset_time(y, input_window=5, output_window=1, stride=1, num_features=1):
    '''
    create a windowed dataset
